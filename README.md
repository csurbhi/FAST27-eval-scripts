...
Following test results are provided in the paper's Evaluation section:
1) fio tests (microbenchmark)
...
start another tmux instance:
	1) cd /home/surbhi/github/FAST27-eval-scripts/Linux-kernel-compile/5ParallelCompiles/host-ls/compile5Times/150GB/7GB-MiddleWaterMark
	2) ./start.sh
	3) This should generate the following log files:
		a) parallel_build_iostat.log
		b) build_[1-5].log
...
### 2. ZFS resilvering tests: (requires around 2 weeks to complete resilvering across all the STLs and the CMR drive)
-----------
...
This also needs some coordination with the authors - as someone at UBC will need to yank a drive
and replace it with the SMR/CMR drive in consideration. This will start the resilvering process
...
### fio tests: (the 1MB tests should take around a day to run for all the 3 STLs. The 4KB tests should take 3-4 days for all the 3 STLs.)

####1 MB test: (should take around 2-3 hours)
...
####4 KB test: (should take around 1-2 days)
1) cd /home/surbhi/github/FAST27-eval-scripts/fio/4KB/90Util/90-10-LBA
2) cd host-hybrid
3) ./start.sh
4) cd ../device-hybrid
5) ./start.sh
...
1) cd /home/surbhi/github/FAST27-eval-scripts/fio/1MB/uniform/90Util
2) cd host-hybrid
3) ./start.sh
4) cd ../device-hybrid
5) ./start.sh
...
####4 KB test: (should take around 2-3 days)
1) cd /home/surbhi/github/FAST27-eval-scripts/fio/4KB/90Util/uniform
2) cd host-hybrid
3) ./start.sh
4) cd ../device-hybrid
5) ./start.sh
...
cd Reverse-Engineering/cache-size
cd 4KB/
fio ./fio-cache-size.fio

Repeat this for every size in the parent "cache-size" directory.


#### Cache medium (should take around 5-6 hours)

cd Reverse-Engineering/cache-medium
cd 90-10-LBA
fio ./fio-cache-size.fio

Repeat this for 80-20-LBA, 70-30-LBA, 60-40-LBA, and uniform.

The python notebook to plot the cache-size is 
Please adjust the location of the log files in this notebook file.
